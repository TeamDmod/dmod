...
const validators = {
  ...passes,
  description({ value }) {
    if (value.length >= DESCRIPTION_MIN && value.length <= DESCRIPTION_MAX_DATA.NORMAL)
      return { error: false };

    const lengthError =
      value.length < DESCRIPTION_MIN
        ? 'too short'
        : value.length > DESCRIPTION_MAX_DATA.NORMAL
        ? 'too long'
  ...
  short_description({ value }) {
    if (value.length >= SHORT_DESCRIPTION_MIN && value.length <= SHORT_DESCRIPTION_MAX_DATA.NORMAL)
      return { error: false };
  ...
  async invite({ value, guildID }) {
    if (!/[a-zA-Z0-9\\-]{2,32}/.test(value))
      return { error: true, message: "Invalid invite doesn't fit regex" };

    const data = await fetch(
      `${API_ENDPOINT}/invites/${value}?with_expiration=true&with_expiration=true`
    ).then(json);

    if (data.message || (data.code && data.code === 10006))
    ...
    const expiresTimestamp =
      _expiresTimestamp ??
      (data.createdTimestamp && data.max_age ? this.createdTimestamp + this.max_age * 1000 : null);

    if (expiresTimestamp == null && expiresTimestamp !== 0)
      return { error: true, message: 'Failed to fetch expiration date' };
    if (expiresTimestamp !== 0 && data.max_uses !== 0)
      return { error: true, message: 'Invalid invite expiration date' };
