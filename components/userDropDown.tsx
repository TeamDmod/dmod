...
export default function UserDropDown({ user }: props) {
  ...
  function userAvatarUrl(): string | null {
    let base = `https://cdn.discordapp.com/avatars/${user.id}/`,
      disc = parseInt(user.discriminator);
      ...
      >
        <Menu.Items className='absolute right-0 w-40 mt-2 bg-dorpdown rounded-md shadow-lg divide-y-3 focus:outline-none text-white'>
          <div className='px-1 py-2'>
            ...
            <Menu.Item>
              {({ active }) => (
                <a className={`${active && 'bg-red-700'} flex rounded-md items-center w-full p-1 text-sm cursor-pointer space-x-3`} onClick={() => router.push('/api/auth/logout')}>
                  <LogoutIcon />
                  <span>Logout</span>
                </a>
              )}
...
function LogoutIcon() {
  return (
    <svg width='16' height='18' viewBox='0 0 16 18' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path d='M5 4L3.6 5.4L6.2 8H-4V10H6.2L3.6 12.6L5 14L10 9L5 4ZM14 16H6V18H14C15.1 18 16 17.1 16 16V2C16 0.9 15.1 0 14 0H6V2H14V16Z' fill='#ADB4C6' />
    </svg>
  );
}
