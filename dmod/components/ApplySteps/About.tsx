...
function About(props) {
	...
	let onFormSubmit = (e) => {
		...
		if (submitter === "forward") {
			if (!data.about) setData({ ...data, about: "" });
			if (!data.description) setData({ ...data, description: "" });
			if (!data.pronouns) setData({ ...data, pronouns: "" });
			if (!data.birthday) setData({ ...data, birthday: "" });
			if (!data.about || !data.description || !data.pronouns || !data.birthday) return
		}
